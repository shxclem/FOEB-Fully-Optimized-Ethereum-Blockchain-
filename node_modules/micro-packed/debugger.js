...
import { EMPTY, _TEST, wrap as coderWrap, utils, } from "./index.js";
const UNKNOWN = '(???)';
...
const DebugReader = /* @__PURE__ */ (() => class DebugReader extends _TEST._Reader {
    debugLst = [];
    cur;
    get lastElm() {
    ...
    finishDebug() {
        ...
        if (!this.debugLst.length)
            this.debugLst.push({ start: 0, end, path: UNKNOWN });
    }
})();
function toBytes(data) {
    ...
    if (!(data.length % 2) && /^[0-9a-f]+$/.test(data))
...
function mapData(lst, data) {
    ...
    for (const elm of lst) {
        ...
        res.push({
            path: elm.path,
            data: Uint8Array.from(data.subarray(elm.start, elm.end)),
            value: elm.value,
        });
        end = elm.end;
    }
...
export function diff(coder, actual, expected, skipSame = true) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (utils.equalBytes(a.data, e.data) && skipSame)
            continue;
