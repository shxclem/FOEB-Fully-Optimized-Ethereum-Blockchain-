import { decodeTx, type ClearSigDef, type ClearSigOpt, type TxDecodeOpt } from '../advanced/abi.ts';
import type { ClearSigTokens } from '../advanced/clearsig.ts';
import { TxVersions, type AccessList } from '../core/tx-internal.ts';
import { type IWeb3Provider, type TArg, type TRet, type Web3CallArgs } from '../utils.ts';
...
export declare class Web3Provider implements IWeb3Provider {
    ...
    ots_searchBefore(address: string, block: number, pageSize?: number): Promise<OtsSearch>;
    ots_searchAfter(address: string, block: number, pageSize?: number): Promise<OtsSearch>;
    ots_traceTransaction(txHash: string): Promise<ActionOts[]>;
    /**
     * OTS API. A bit broken: when >= 25 txs/block per tx, we can't get more than 25 = we will miss
     * information!
     */
    internalTransactionsOTS(address: string, opts?: TraceOpts): Promise<Action[]>;
    ...
    private txInfoRaw;
    txInfo(txHash: string, opts?: TxInfoOpts): Promise<TxInfoFull>;
     ...
     */
    discoverTx(transaction: string, clearSig?: Record<string, ClearSigDef>, opt?: TArg<TxDecodeOpt & {
        tokens?: ClearSigTokens;
