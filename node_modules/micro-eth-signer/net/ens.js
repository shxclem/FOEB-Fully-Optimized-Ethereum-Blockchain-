...
import { createContract } from "../advanced/abi-decoder.js";
import { astring, strip0x } from "../utils.js";
...
export function namehash(address) {
    ...
    for (let label of address.split('.').reverse())
        res = keccak_256(concatBytes(res, keccak_256(utf8ToBytes(label.toLowerCase()))));
    return res;
}
export default class ENS {
    ...
    async addressToName(address) {
        ...
        if (!realAddr || strip0x(realAddr).toLowerCase() !== strip0x(address).toLowerCase())
            return;
        return name;
    }
}
