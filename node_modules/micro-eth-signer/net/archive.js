import { ERC1155, ERC20, ERC721, WETH, CLEARSIG_REPO, addTokens, createContract, decodeTx, events, } from "../advanced/abi.js";
import { TxVersions, legacySig } from "../core/tx-internal.js";
import { Transaction } from "../core/tx.js";
import { amounts, ethHex, ethHexNum, } from "../utils.js";
/*
...
*/
// Utils
const ethNum = (n) => ethHexNum.encode(n === undefined ? 0 : n);
...
const TOKEN_TRANSFER_TOPICS = /* @__PURE__ */ (() => new Set([
...
const validateToken = (token, name) => {
    if (token === null || typeof token !== 'object')
...
const fixOtsSearch = (search) => {
    const receipts = {};
    for (const r of search.receipts)
        receipts[r.transactionHash] = fixTxReceipt(r);
    return {
...
function validatePagination(opts) {
    for (const i of ['fromBlock', 'toBlock']) {
        ...
        if (val === undefined || (typeof val === 'number' && Number.isSafeInteger(val) && val >= 0))
            continue;
        throw new Error(`validatePagination: wrong field ${i}=${opts[i]}. Should be non-negative integer or undefined`);
    }
}
function validateTraceOpts(opts) {
    ...
    for (const i of ['perRequest', 'perRequestOTS', 'limitTrace']) {
        ...
        if (val === undefined || (typeof val === 'number' && Number.isSafeInteger(val) && val > 0))
            continue;
        throw new Error(`validateTraceOpts: wrong field ${i}=${opts[i]}. Should be positive integer or undefined`);
    }
...
function validateLogOpts(opts) {
    ...
    for (const i of ['limitLogs']) {
        ...
        if (val === undefined || (typeof val === 'number' && Number.isSafeInteger(val) && val > 0))
            continue;
        throw new Error(`validateLogOpts: wrong field ${i}=${opts[i]}. Should be positive integer or undefined`);
    }
...
export class Web3Provider {
    ...
    async ots_searchBefore(address, block, pageSize = 25) {
        if (typeof address !== 'string')
            throw new Error('ots_searchBefore: wrong address');
        // OtterScan search block arguments are chain heights; negative heights wrap badly server-side.
        if (!Number.isSafeInteger(block) || block < 0)
            throw new Error('ots_searchBefore: wrong block');
    ...
    async ots_searchAfter(address, block, pageSize = 25) {
        if (typeof address !== 'string')
            throw new Error('ots_searchAfter: wrong address');
        // OtterScan search block arguments are chain heights; negative heights wrap badly server-side.
        if (!Number.isSafeInteger(block) || block < 0)
            throw new Error('ots_searchAfter: wrong block');
        // OtterScan pageSize is a pagination count; non-positive counts cannot make progress.
        if (!Number.isSafeInteger(pageSize) || pageSize <= 0)
    ...
    async ots_traceTransaction(txHash) {
        if (typeof txHash !== 'string')
            throw new Error('ots_traceTransaction: wrong txHash');
     ...
     * information!
     */
    async internalTransactionsOTS(address, opts = {}) {
        if (typeof address !== 'string')
        ...
        for (;;) {
            ...
            for (const t of res.txs) {
                ...
                if (opts.txInfoCallback)
                    opts.txInfoCallback(hash, txInfo);
                if (!traces[hash])
                    traces[hash] = this.ots_traceTransaction(hash);
        ...
        for (const hash in tracesRes) {
            ...
            for (const a of actions) {
                if (a.from !== addrLow && a.to !== addrLow)
                    continue; // filter unrelated traces
    ...
    async internalTransactions(address, opts = {}) {
        ...
        if (opts.limitTrace) {
            ...
            for (let i = opts.fromBlock; i <= opts.toBlock; i += opts.limitTrace + 1)
                promises.push(this.traceFilterSingle(address, {
                    fromBlock: i,
    ...
    async ethLogs(topics, opts = {}) {
        ...
        for (let i = fromBlock; i <= opts.toBlock; i += opts.limitLogs)
            promises.push(this.ethLogsSingle(topics, {
                fromBlock: i,
    ...
    async wethTransfers(address, opts = {}) {
        if (typeof address !== 'string')
            throw new Error('wethTransfers: wrong address');
        validateLogOpts(opts);
    ...
    async erc1155Transfers(address, opts = {}) {
        if (typeof address !== 'string')
            throw new Error('erc1155Transfers: wrong address');
        validateLogOpts(opts);
        return await Promise.all([
            // Single
    ...
    async tokenBalanceSingle(address, token, tokenIds) {
        ...
        if (token.abi === 'ERC20') {
            ...
            if (!token.enumerable) {
                ...
                return new Map(ids.map((i, j) => [i, owners[j].toLowerCase() === address.toLowerCase() ? _1n : _0n]));
            }
    ...
    async tokenBalances(address, tokens, tokenIds) {
        ...
        for (let i = 0; i < _tokens.length; i++) {
            ...
            if (!isError(token))
                _tokens[i] = validateToken(token, 'tokenBalances');
        }
    ...
    async transfers(address, opts = {}) {
        ...
        const blockCache = {};
        const tokenCache = {};
        const loadToken = (contract) => {
            if (tokenCache[contract])
                return;
            tokenCache[contract] = this.tokenInfo(contract);
        };
        const _opts = {
            ...opts,
            txInfoCallback: (txHash, info) => {
                if (txCache[txHash])
                    return;
                txCache[txHash] = info;
            },
        ...
        for (const hash in tx) {
            for (const log of tx[hash].receipt.logs) {
                ...
                if (TOKEN_TRANSFER_TOPICS.has(log.topics[0]))
                    loadToken(log.address);
            }
        }
    ...
    async allowances(address, opts = {}) {
        if (typeof address !== 'string')
            throw new Error('allowances: wrong address');
     ...
     */
    async discoverTx(transaction, clearSig = CLEARSIG_REPO, opt = {}) {
        ...
        if (tx.raw.to !== '0x' && !tokens[to]) {
            ...
            if (!('error' in info)) {
                const token = { abi: info.abi, chainId: tx.raw.chainId };
                if ('name' in info)
    ...
    clearSigCallbacks() {
        ...
        return {
            async resolveToken(req) {
                ...
                if ('error' in info || info.abi !== 'ERC20')
                    return undefined;
            ...
            async resolveNft(req) {
                ...
                if ('error' in info || info.abi !== 'ERC721' || !info.name)
                    return undefined;
...
export function calcTransfersDiff(transfers) {
    ...
    for (const t of transfers) {
        ...
        Object.assign(t, {
            balances: { ...balances },
            // deep copy
            tokenBalances: Object.fromEntries(Object.entries(tokenBalances).map(([k, v]) => [
                k,
                Object.fromEntries(Object.entries(v).map(([addr, balances]) => [addr, new Map(balances)])),
            ])),
        });
    }
    return transfers;
