...
import { amounts, astring, deepFreeze, ethHex, initSig, isBytes, isObject, sign, strip0x, } from "../utils.js";
    ...
    const map = (items, fn) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (!Object.hasOwn(items, i))
                throw new Error(`missing array item ${i}`);
            res.push(fn(items[i]));
        }
        return res;
    };
...
const mkYParityCoder = /* @__PURE__ */ () => P.coders.reverse(P.validate(P.int(1, false, false, false), (elm) => {
    assertYParityValid(elm);
    return elm;
}));
const mkAccessListItem = /* @__PURE__ */ () => struct({ address: addrCoder, storageKeys: array(Bytes32) });
export const authorizationRequest = /* @__PURE__ */ struct({
    chainId: U256BE,
    address: addrCoder,
    nonce: U64BE,
});
const mkAuthorizationItem = /* @__PURE__ */ () => struct({
    chainId: U256BE,
    address: addrCoder,
    nonce: U64BE,
...
// prettier-ignore
const legacyInternal = /* @__PURE__ */ txStruct(['nonce', 'gasPrice', 'gasLimit', 'to', 'value', 'data'], ['v', 'r', 's']);
...
export const RawTx = /* @__PURE__ */ (() => P.apply(createTxMap(TxVersions), {
 ...
 */
export const RlpTx = /* @__PURE__ */ (() => createTxMap(Object.fromEntries(Object.keys(TxVersions).map((k) => [k, RLP]))))();
// prettier-ignore
...
export const calcIntrinsicGas = (type, data) => {
...
const validators = {
    nonce(num, { strict }) {
        ...
        // EIP-2681 §Specification: transactions are invalid when nonce >= 2**64 - 1.
            ...
            minmax(num, _0n, amounts.maxUint64 - BigInt(1));
    },
    maxFeePerGas(num, { strict, data }) {
        abig(num);
        if (strict)
            minmax(num, BigInt(1), amounts.maxGasPrice, '>= 1 wei and < 10000 gwei');
        else
            minmax(num, _0n, amounts.maxUint256);
    ...
    gasLimit(num, { strict, type, data }) {
        ...
        if (strict) {
            ...
            if (num < min)
                throw new Error(`intrinsic gas too low: ${num} < ${min}`);
        }
        else
            minmax(num, _0n, amounts.maxUint64);
    },
    to(address, { strict, type, data }) {
    ...
    chainId(num, { strict, type, data }) {
        ...
        if (strict) {
            ...
            const max = type === 'legacy' && (data.yParity === 0 || data.yParity === 1)
                ? amounts.maxChainId
                : amounts.maxUint256;
    ...
    blobVersionedHashes(list, { strict }) {
        ...
        if (strict && list.length === 0)
            throw new Error('must contain at least one versioned hash');
        for (let i = 0; i < list.length; i++) {
            ...
            if (firstByte.toLowerCase() !== '01')
                throw new Error('versioned hash must start with 0x01');
        }
    },
    authorizationList(list, opts) {
        ...
        for (const { address, nonce, chainId } of list) {
            ...
            abig(chainId);
            if (opts.strict)
                minmax(chainId, _0n, amounts.maxUint256, '>= 0 and < 2**256');
...
export function validateFields(type, data, strict = true, allowSignatureFields = true) {
    ...
    function checkField(field) {
        ...
        try {
            ...
            if (type === 'legacy' && field === 'chainId' && val === undefined)
                return;
...
export const authorization = /* @__PURE__ */ deepFreeze({
    _getHash(req) {
    ...
    getAuthority(item) {
        if (!isObject(item))
            throw new TypeError('"item" expected object, got type=' + typeof item);
