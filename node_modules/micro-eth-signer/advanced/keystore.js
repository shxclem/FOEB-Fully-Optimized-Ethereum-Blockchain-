...
import { deepFreeze, isBytes, strip0x } from "../utils.js";
...
function assertUint32(index) {
    if (typeof index !== 'number')
        throw new TypeError('Expected uint32 number');
 ...
 * ```
 */
export function hkdfModR(ikm, keyInfo = Uint8Array.of()) {
    ikm = abytes(ikm, undefined, 'ikm');
    // ERC-2333 hkdf_mod_r input requires IKM to be a secret octet string >= 256 bits.
    if (ikm.length < 32)
        throw new RangeError('Expected ikm to be at least 32 bytes');
...
export function deriveMaster(seed) {
    ...
    if (seed.length < 32)
        throw new RangeError('Expected seed to be at least 32 bytes');
...
export function deriveSeedTree(seed, path) {
    if (typeof path !== 'string')
        throw new TypeError('Derivation path must be string');
    const indices = path.split('/');
    if (indices.shift() !== 'm')
        throw new RangeError('First character of path must be "m"');
    // ERC-2334 Path has four levels plus the master node and says at least four
    // child levels MUST be used.
    if (indices.length < 4)
        throw new RangeError('Derivation path must have at least four levels');
    ...
    const nodes = indices.map((i) => {
        ...
        if (!/^\d+$/.test(i))
            throw new RangeError('Derivation path segment must be an integer');
        return Number(i);
    });
...
export const EIP2334_KEY_TYPES = /* @__PURE__ */ deepFreeze([
    'withdrawal',
    'signing',
]);
/**
...
function deriveEIP2335Key(password, salt, kdf, params = KDFS[kdf]) {
    ...
    if (kdf === 'scrypt') {
        const { n: N, r, p, dklen: dkLen } = params;
        return scrypt(pass, salt, { N, r, p, dkLen });
    }
    else if (kdf === 'pbkdf2') {
        const { c, dklen: dkLen, prf } = params;
        if (prf !== 'hmac-sha256')
...
export class EIP2335Keystore {
    ...
    description = '', pubkey) {
        ...
        if (pubkey !== undefined)
            res.pubkey = bytesToHex(abytes(pubkey, undefined, 'public key'));
        return res;
    }
    /**
...
export const _TEST = /* @__PURE__ */ deepFreeze({ normalizePassword, deriveEIP2335Key });
const fromHex = (hex) => hexToBytes(strip0x(hex));
function addrFromPriv(privateKey) {
    return addr.fromPrivateKey(privateKey).slice(2).toLowerCase();
}
function assertPriv(privateKey) {
    if (!secp256k1.utils.isValidSecretKey(privateKey))
        throw new Error('invalid private key');
    return Uint8Array.from(privateKey);
}
function assertCipher(cipher) {
    if (cipher !== 'aes-128-ctr' && cipher !== 'aes-128-cbc') {
        throw new Error(`Unsupported cipher: ${cipher}`);
    }
}
function prepareBytes(name, value, randomLength, hexLength) {
    if (value === undefined)
        return randomBytes(randomLength);
    if (typeof value === 'string') {
        const hex = strip0x(value);
        const expectsLen = hexLength !== undefined;
        if (hex.length === 0 && !expectsLen)
            return Uint8Array.of();
        if (hex.length % 2 !== 0 || (expectsLen && hex.length !== hexLength))
            throw new Error(`invalid ${name}: expected string or Uint8Array of length ${hexLength ?? randomLength}`);
...
function initCipher(cipher, key, iv, pkcs7Padding = false) {
    assertCipher(cipher);
    return cipher === 'aes-128-ctr' ? ctr(key, iv) : cbc(key, iv, { disablePadding: !pkcs7Padding });
}
function assertDklen(dklen) {
...
async function deriveKey(password, kdf, params) {
    ...
    if (kdf === 'scrypt') {
        const { salt, n: N, r, p, dklen: dkLen } = params;
        assertDklen(dkLen);
        return scryptAsync(pass, fromHex(salt), { N, r, p, dkLen });
    }
    if (kdf === 'pbkdf2') {
        const { salt, c, dklen: dkLen, prf } = params;
        if (prf !== 'hmac-sha256')
...
function lowerCase(value) {
    ...
    if (Array.isArray(value))
        return value.map(lowerCase);
    if (value && typeof value === 'object') {
        return Object.fromEntries(Object.entries(value).map(([key, val]) => [key.toLowerCase(), lowerCase(val)]));
    }
    return value;
}
function prepEncKdf(params) {
 ...
 * ```
 */
export async function privFromLegacyKeystore(keystore, password, nonStrict = false) {
    if (typeof keystore !== 'object')
        throw new Error('expected parsed json');
    const keyst = nonStrict ? lowerCase(keystore) : keystore;
    if (keyst.version !== 3)
        throw new Error('Not a V3 keystore');
 ...
 * ```
 */
export async function privToLegacyKeystore(privKey, password, opts) {
    ...
    try {
        const ciphertext = initCipher(params.cipher, derivedKey.subarray(0, 16), params.iv).encrypt(key);
        const mac = keccak_256(concatBytes(derivedKey.subarray(16, 32), ciphertext));
        return {
            version: 3,
 ...
 * ```
 */
export async function privFromLegacySaleKeystore(keystore, password) {
    ...
    try {
        try {
            seed = initCipher('aes-128-cbc', derivedKey.subarray(0, 16), encseed.subarray(0, 16), true).decrypt(encseed.subarray(16));
        }
        catch (error) {
