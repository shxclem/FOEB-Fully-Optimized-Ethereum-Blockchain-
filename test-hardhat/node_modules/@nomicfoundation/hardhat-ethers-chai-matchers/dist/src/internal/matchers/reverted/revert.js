...
import { decodeReturnData, getReturnDataFromError, hasTransactionHash, isNoDataExecutionError, parseBytes32String, } from "./utils.js";
    ...
    Assertion.addMethod(REVERT_MATCHER, function (ethers) {
        ...
        const onSuccess = async (value) => {
            ...
            if (hasTransactionHash(value) || typeof value === "string") {
                ...
                if (!isHash(hash)) {
                    chaiAssert.fail(`Expected a valid transaction hash, but got "${String(hash)}"`);
                }
                ...
                if (typeof value === "string" && isBytes32String(hash)) {
                    const oneShotReceipt = await ethers.provider.getTransactionReceipt(hash);
                    if (oneShotReceipt === null) {
                        assert(false, "Expected transaction to be reverted", "Expected transaction NOT to be reverted");
                        return;
                    }
                    assert(oneShotReceipt.status === 0, "Expected transaction to be reverted", "Expected transaction NOT to be reverted");
                    return;
                }
                const receipt = await waitForTransactionReceipt(ethers, hash);
                assertIsNotNull(receipt, "Transaction's receipt cannot be fetched from the network");
...
async function waitForTransactionReceipt(ethers, hash) {
