...
import { decodeReturnData, ErrorWithData, getReturnDataFromError, getTransactionRevertData, resultToArray, throwRevertDataNotRetrievedError, } from "./utils.js";
    ...
    Assertion.addMethod(REVERTED_WITH_CUSTOM_ERROR_MATCHER, function (contract, expectedCustomErrorName, ...args) {
        ...
        const onSuccess = async (value) => {
            ...
            if (revertData.kind === "Revert") {
                ...
                throwRevertDataNotRetrievedError(`Expected transaction to be reverted with custom error '${expectedCustomErrorName}', but the revert data couldn't be retrieved`, revertData.retrievalError);
            }
...
function validateInput(obj, contract, expectedCustomErrorName, args) {
    try {
        ...
        if (typeof contract === "string" || contract?.interface === undefined) {
            ...
            chaiAssert.fail("The first argument of .revertedWithCustomError must be the contract that defines the custom error");
        }
        ...
        if (expectedCustomError === null) {
            chaiAssert.fail(`The given contract doesn't have a custom error named "${expectedCustomErrorName}"`);
        }
        if (args.length > 0) {
            chaiAssert.fail("The .revertedWithCustomError matcher expects two arguments: the contract and the custom error name. Arguments should be asserted with the .withArgs helper.");
        }
...
export async function revertedWithCustomErrorWithArgs(context, Assertion, _chaiUtils, expectedArgs, ssfi) {
    ...
    if (customErrorAssertionData === undefined) {
        chaiAssert.fail("[.withArgs] should never happen, please submit an issue to the Hardhat repository");
    }
    ...
    assertIsNotNull(errorFragment, "Error type can't be found in the contract's interface");
