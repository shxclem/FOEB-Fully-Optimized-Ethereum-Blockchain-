...
import { decodeReturnData, ErrorWithData, getReturnDataFromError, getTransactionRevertData, throwRevertDataNotRetrievedError, } from "./utils.js";
export function supportRevertedWithPanic(Assertion, chaiUtils) {
    Assertion.addMethod(REVERTED_WITH_PANIC_MATCHER, function (expectedCodeArg) {
        ...
        try {
            ...
            try {
                chaiAssert.fail(`Expected the given panic code to be a number-like value, but got "${expectedCodeArg}"`);
            }
            catch (e) {
                ensureError(e);
                e.cause = cause;
        ...
        const onSuccess = async (value) => {
            ...
            if (revertData.kind === "Revert") {
                ...
                throwRevertDataNotRetrievedError(`Expected transaction to be reverted with ${formattedPanicCode}, but the revert data couldn't be retrieved`, revertData.retrievalError);
            }
